...
8. select e.ename, e.salary from employess e where e.eid not in ( select distinct c.eid from certified c ) and e.salary >( select avg (e1.salary)
   from employess e1 where e1.eid in ( select distinct c1.eid from certified c1 ) );
